<template>
  <div class="warpper">
    <div class="todo_header">
      ...
      <button @click="handleClickDispose">handleClickDispose</button>
      <button @click="handleClickState">handleClickState</button>
    </div>
    <div class="todo_main">
      <h2>todolist length: {{ store.length }}</h2>
      <ul class="list">
        <li v-for="todo of store.todolist1"
            :key="todo.id">
          <input type="checkbox"
          ...
          <button type="button"
                  @click="removeTodo(todo.id)">RemoveTodo</button>
        </li>
      </ul>
    </div>
  </div>
